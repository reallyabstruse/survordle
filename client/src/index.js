...
class Game extends React.Component {
  constructor(props) {	  
	...
	this.duel = window.location.pathname === "/duel";
	...
	if (this.duel && this.state.gameId && this.state.playerId) {
		this.sendJson(null);
	}
	  ...
	  if (data.guessResult) {
		  this.setState({
			  ...
			  guesses: [...this.state.guesses,
						data.guessResult.word],
			  guessColors: [...this.state.guessColors,
							data.guessResult.colors]
		  });
  ...
  validWord(guess) {
    return possibleSolutions.includes(guess) || wordList.includes(guess);
  }
  
  submitGuess() {
    // No submitting if game has ended or word removal animation is in progress
    if (this.state.gameover || this.state.showSettings || this.state.wordsToRemove) {
      return;
    }
    ...
    if (guess === this.state.solution) {
      ...
      this.setState(
        ...
        () => {
          this.setState({
            guessColors: [
              ...this.state.guessColors.slice(0, this.state.wordsToRemove),
              ...this.state.guesses
                .slice(this.state.wordsToRemove)
                .map((item, index) => {
                  return this.getColors(item);
                })
            ]
          });
        }
  ...
  getColors(guess) {
    ...
    if (this.state.solution) {
      let arr_guess = [...guess];
      let arr_solution = [...this.state.solution];

      for (let i = 0; i < arr_solution.length; i++) {
        if (arr_guess[i] === arr_solution[i]) {
          this.addKeyboardGreen(arr_guess[i]);
      ...
      for (let i = 0; i < arr_guess.length; i++) {
        ...
        for (let j = 0; j < arr_solution.length; j++) {
          if (arr_guess[i] === arr_solution[j]) {
            this.addKeyboardYellow(arr_guess[i]);
      ...
      for (let i = 0; i < arr_guess.length; i++) {
        if (arr_guess[i]) {
          this.addKeyboardBlack(arr_guess[i]);
          colors[i] = WHITE;
        }
      }
    }
  ...
  addKeyboardGreen(key) {
	this.addKeyboardColor(key, GREEN);
  }

  addKeyboardBlack(key) {
    if (!this.state.keyboardColors[key]) {
      this.addKeyboardColor(key, BLACK);
    }
  }

  addKeyboardYellow(key) {
    if (!this.state.keyboardColors[key]) {
      this.addKeyboardColor(key, YELLOW);
    }
  }
  ...
  render() {
    ...
    let rows = [];
    // Previous guesses
    for (let i in this.state.guesses) {
      rows.push(
        <GuessRow
          val={this.state.guesses[i]}
          key={i}
    ...
    if (this.state.guesses.length < this.state.maxguesses + this.state.wordsToRemove) {
      ...
      for (
        ...
        rows.push(<GuessRow type="future" val="" key={i} />);
      }
    }
    ...
    return (
      <div className={classNames("game", { dark: this.state.settings.dark })}>
        ...
        <div
          className={classNames("guessescontainer", {
            gameover: this.state.gameover
          })}
        >
          <div
            className={classNames("message", {
              hidemessage: !this.state.showmessage,
              [this.state.messagecolor]: this.state.showmessage
            })}
          >
            {this.state.message}
          </div>
          <div
            className={classNames("guessesinner", {
              removewords: this.state.wordsToRemove
            })}
            style={{
